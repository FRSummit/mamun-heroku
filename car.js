	...
	if(carCode != "Select one"){
		...
		$.getJSON(url, function(json) {
			...
			for (var i = 0; i < json.records.length-1; i++) {
				console.log(json);
				var dateSplit = json.records[i].Date.split("T");
				var rearrangeDate = dateSplit[0].split("-");
				var realDate = rearrangeDate[1] + "/" + (parseInt(rearrangeDate[2])+1) + "/" + rearrangeDate[0];
				tr = table1.insertRow(-1);
				var tabCell = tr.insertCell(-1);
				tabCell.innerHTML = json.records[i].ID;
				tabCell = tr.insertCell(-1);
				//tabCell.innerHTML = json.records[i].Date;
				tabCell.innerHTML = realDate;
				tabCell = tr.insertCell(-1);
